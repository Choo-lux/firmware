...
if [ "${curl_result}" -eq 0 ]; then
	if grep -q "." /tmp/upgrade/version; then
		if [ "$(uci get wifimesh.system.version)" != "$(cat /tmp/upgrade/version)" ]; then
			# Make sure no old firmware exists
			if [ -e "/tmp/firmware.bin" ]; then rm "/tmp/firmware.bin"; fi
			
			echo "Checking for upgrade binary"
			if grep -q "$(cat /tmp/sysinfo/board_name)-squashfs-sysupgrade" /tmp/upgrade/md5sums; then
				...
				curl -A "WMF/v$(uci get wifimesh.system.version) (http://www.wifi-mesh.co.nz/)" --cacert /etc/ssl-bundle.crt -s -o /tmp/firmware.bin "${url}$(grep $(cat /tmp/sysinfo/board_name)'-squashfs-sysupgrade' /tmp/upgrade/md5sums | awk '{ print $2 }' | sed 's/*//')?r=$(head -30 /dev/urandom | tr -dc "0123456789" | head -c3)" > /dev/null
				...
				elif [ "$(grep $(cat /tmp/sysinfo/board_name)'-squashfs-sysupgrade' /tmp/upgrade/md5sums | awk '{ print $1 }')" = "$(md5sum /tmp/firmware.bin | awk '{ print $1 }')" ]; then
